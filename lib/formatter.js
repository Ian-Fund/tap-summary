...
function Formatter(opts) {
  ...
  this.needStack =  opts.stack !== false
...
Formatter.prototype.prettifyError = function (assertion) {
  ...
  var rawErrorLines = rawError.split(LF)
  if (!this.needStack) {
    var stack = assertion.error.stack
    var stackLines = stack.split(LF)
    var numLines = stackLines.length
    rawErrorLines = rawErrorLines.slice(0, rawErrorLines.length - numLines)
  }
  return this.format(rawErrorLines.join(LF), 'cyan')
}
